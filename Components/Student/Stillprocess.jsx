...
export default function RestaurantPortalDashboard() {
  ...
  const startCooking = (orderId) => {
    if (!can.confirm) return showToast("You don't have permission to confirm orders", "error");
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: "cooking", assignedTo: currentUser?.id } : o));
    ...
    try {
      ...
      order.items.forEach(item => {
        ...
        Object.entries(recipe).forEach(([ingName, qtyPerPlate]) => {
          ...
          ledger.push({ 
            id: uid("stockout"), ingredient: ingName, type: "out", quantity: total, 
            unitCost: ing.unitCost, refType: "order", refId: order.id, orderNumber: order.orderNumber,
            createdAt: Date.now(), recordedBy: currentUser?.id 
          });
        });
      });
      setIngredients(ingClone);
      setStockLedger(prev => [...ledger, ...prev]);
      const low = ingClone.filter(i => Number(i.qty) <= Number(i.reorderLevel));
      low.forEach(ing => pushNotif("Low Stock Alert", `${ing.name} is low (${ing.qty} ${ing.unit})`, "warn", "high"));
  ...
  const markServed = (orderId) => {
    if (!can.serve) return showToast("You don't have permission to serve orders", "error");
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    setOrders(prev => prev.map(o => {
      ...
      setConsumptionLog(prevLog => [...prevLog, {
        id: uid("cons"), date: todayISO(), orderId: o.id, orderNumber: o.orderNumber, plates: platesCount, worth, items: o.items
      }]);
  ...
  const markCompleted = (orderId) => {
    if (!can.complete) return showToast("You don't have permission to complete orders", "error");
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
  ...
  const cancelOrder = (orderId, reason = "") => {
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: "canceled", cancelReason: reason, canceledAt: Date.now() } : o));
    pushNotif("Order Canceled", `Order ${order.orderNumber} canceled`, "warn");
    logAudit("CANCEL_ORDER", `Canceled order ${order.orderNumber}: ${reason}`);
    showToast("Order canceled", "warning");
  };

  const deleteOrder = (orderId) => {
    if (!can.deleteOrder) return showToast("You don't have permission to delete orders", "error");
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    setConfirmDialog({
      title: "Delete Order",
  ...
  const receivePayment = (orderId, method = "cash", amount = null) => {
    if (!can.receivePayment) return showToast("You don't have permission to receive payments", "error");
    const order = orders.find(o => o.id === orderId);
    if (!order) return showToast("Order not found", "error");
    const paymentAmount = amount || calcOrderTotal(order);
    setOrders(prev => prev.map(o => o.id === orderId ? { ...o, paid: true, paymentMethod: method, paidAt: Date.now() } : o));
    setTransactions(prev => [{ 
      id: uid("tx"), type: "sale", amount: paymentAmount, orderId,
      orderNumber: order.orderNumber, method, createdAt: Date.now(), recordedBy: currentUser?.id
    }, ...prev]);
    if (order.customerPhone) {
      setCustomers(prev => prev.map(c => c.phone === order.customerPhone ? { ...c, totalSpent: (c.totalSpent || 0) + paymentAmount } : c));
    }
    pushNotif("Payment Received", `Payment of RWF ${formatAmount(paymentAmount)} received for ${order.orderNumber}`, "success", "high");
    logAudit("RECEIVE_PAYMENT", `Payment of RWF ${formatAmount(paymentAmount)} for ${order.orderNumber}`);
  ...
  const recordDelivery = (ingredient, quantity, unitCost, supplier = "") => {
    if (!can.stockIn) return showToast("You don't have permission to manage stock", "error");
    setIngredients(prev => prev.map(i => i.name === ingredient ? { ...i, qty: Number(i.qty) + quantity, lastRestocked: Date.now() } : i));
    setStockLedger(prev => [{ 
      id: uid("stockin"), ingredient, type: "in", quantity, unitCost, supplier,
      refType: "delivery", refId: uid("del"), createdAt: Date.now(), recordedBy: currentUser?.id
    }, ...prev]);
    pushNotif("Stock In", `${ingredient} +${quantity}${ingredients.find(i => i.name === ingredient)?.unit || ""} recorded`, "info");
    logAudit("STOCK_IN", `Recorded delivery of ${quantity}${ingredients.find(i => i.name === ingredient)?.unit || ""} ${ingredient} from ${supplier || 'Unknown supplier'}`);
    showToast(`Stock updated: ${ingredient} +${quantity}`, "success");
  };

  // ==================== SUBSCRIPTION OPERATIONS ====================

  const createSubscription = (customerData, amount = 35000, mealsPlan = 30) => {
    if (!can.manageSubscriptions) return showToast("Permission denied", "error");
    const subscription = { 
      id: uid("sub"), 
    ...
    setTransactions(prev => [{ 
      id: uid("tx"), type: "subscription", amount, subscriptionId: subscription.id,
      customerName: customerData.name, method: "momo", createdAt: Date.now(), recordedBy: currentUser?.id
    }, ...prev]);
  ...
  const renewSubscription = (subscriptionId) => {
    ...
    setSubscriptions(prev => prev.map(s => {
      if (s.id === subscriptionId) {
        const renewed = { ...s, startDate: Date.now(), endDate: Date.now() + 30 * 86400000, status: "active", mealsUsed: 0 };
        setTransactions(prevTx => [{ 
          id: uid("tx"), type: "subscription", amount: s.amount, subscriptionId: s.id,
          customerName: s.customerName, method: "momo", createdAt: Date.now(), recordedBy: currentUser?.id
        }, ...prevTx]);
  ...
  const toggleMenuAvailability = (id) => {
    if (!can.editMenu) return showToast("Permission denied", "error");
  ...
  const updateMenuPrice = (id, price) => {
    ...
    if (p < 0) return showToast("Price cannot be negative", "error");
    const item = menu.find(m => m.id === id);
    if (!item) return;
  ...
  const addMenuItem = (menuItem) => {
    if (!can.editMenu) return showToast("Permission denied", "error");
    const newItem = { id: uid("menu"), ...menuItem, available: true };
  ...
  const generatePerformanceReport = () => {
    const dailyOrdersLocal = orders.filter(o => sameDay(new Date(o.createdAt), new Date(selectedDate)));
    const dailyServedLocal = dailyOrdersLocal.filter(o => ["served", "completed"].includes(o.status));
    const dailyWorthLocal = dailyServedLocal.reduce((s, o) => s + calcOrderTotal(o), 0);
    const report = {
      reportId: uid("report"),
      ...
      daily: {
        ...
        worth: dailyWorthLocal,
        ordersCount: dailyOrdersLocal.length,
        completedOrders: dailyServedLocal.length,
      },
      monthly: {
      ...
      inventory: {
        ...
        stockMovements: stockLedger.filter(s => s.createdAt >= monthFilterRange.start && s.createdAt <= monthFilterRange.end).length,
      },
      customers: {
  ...
  const exportPerformanceReport = () => {
    ...
    report.topSellingItems.forEach((item, idx) => {
      rows.push({ category: "Top Selling Items", metric: `#${idx + 1} ${item.name}`, value: `${item.quantity} orders` });
    });
  ...
  const highPriorityNotifications = useMemo(() => notifications.filter(n => n.priority === "high" && !n.read), [notifications]);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 flex">
      ...
      {confirmDialog && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-xl p-6 max-w-md w-full">
            <h3 className="font-bold text-lg mb-2">{confirmDialog.title}</h3>
            <p className="text-gray-600 mb-6">{confirmDialog.message}</p>
            <div className="flex gap-3">
              <button onClick={confirmDialog.onConfirm} className="flex-1 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Confirm</button>
              <button onClick={confirmDialog.onCancel} className="flex-1 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Cancel</button>
            </div>
          </motion.div>
        </div>
      )}
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside initial={{ x: -300 }} animate={{ x: 0 }} exit={{ x: -300 }} className="w-64 bg-white shadow-lg fixed left-0 top-0 bottom-0 z-20 overflow-y-auto">
            ...
            <nav className="p-4">
              {[
                ...
                .map(item => (
                  <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg mb-2 transition-colors ${activeTab === item.id ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100"}`}>
                    <item.icon />
        ...
        <header className="bg-white shadow-sm px-4 lg:px-6 py-3 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebarOpen(v => !v)} className="p-2 hover:bg-gray-100 rounded-lg">
                <FaBars />
              </button>
              <h1 className="text-xl font-bold capitalize">{activeTab === "dashboard" ? "Dashboard Overview" : activeTab}</h1>
            </div>
            <div className="flex items-center gap-3">
              ...
              <select value={role} onChange={e => setRole(e.target.value)} className="px-3 py-2 rounded-lg text-sm border">
                <option value="manager">Manager</option>
                <option value="chef">Chef</option>
                <option value="cashier">Cashier</option>
                <option value="waiter">Waiter</option>
              </select>
              <button onClick={() => setShowNotifs(v => !v)} className="relative p-2 hover:bg-gray-100 rounded-lg">
            ...
            >
              <div className="flex items-center justify-between p-4 border-b">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={markAllNotificationsRead} className="text-sm text-blue-600 hover:text-blue-700">
                    Mark all read
                  </button>
                  <button onClick={clearAllNotifications} className="text-sm text-red-600 hover:text-red-700">
                    Clear all
                  </button>
                  <button onClick={() => setShowNotifs(false)} className="p-2 hover:bg-gray-100 rounded-lg">
                    <FaTimes />
                  </button>
                </div>
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <div className="grid md:grid-cols-4 gap-4">
          <div>
            ...
            <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="w-full border rounded-lg px-3 py-2" />
          </div>
          <div>
            <label className="text-sm text-gray-500 block mb-1">Month</label>
            <select value={selectedMonth} onChange={e => setSelectedMonth(Number(e.target.value))} className="w-full border rounded-lg px-3 py-2">
              {monthNames.map((m, i) => (<option key={i} value={i}>{m}</option>))}
            </select>
          </div>
          <div>
            <label className="text-sm text-gray-500 block mb-1">Year</label>
            <input type="number" value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))} className="w-full border rounded-lg px-3 py-2" min="2020" max="2030" />
          </div>
          <div>
            <label className="text-sm text-gray-500 block mb-1">Quick Actions</label>
            <button onClick={() => { createOrder({ customerName: "Walk-in", tableNumber: 1, orderType: "dine-in", items: [] }); }} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard icon={FaUtensils} label="Plates Served Today" value={dailyPlatesServed} subtitle={`${dailyConsumption.length} orders`} color="blue" />
        <KPICard icon={FaMoneyBill} label="Daily Worth" value={`RWF ${formatAmount(dailyWorth)}`} subtitle="Revenue from served meals" color="green" />
        <KPICard icon={FaUserFriends} label="Active Subscribers" value={activeSubscribers} subtitle={`${props.monthlyNewSubs || 0} new this month`} color="purple" />
        <KPICard icon={FaChartLine} label="Monthly Revenue" value={`RWF ${formatAmount(monthlyTotalRevenue)}`} subtitle={`${revenueGrowthPct >= 0 ? '+' : ''}${revenueGrowthPct}% growth`} color={revenueGrowthPct >= 0 ? "green" : "red"} growth={revenueGrowthPct} />
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <h3 className="font-bold mb-3 flex items-center gap-2"><FaClipboardList className="text-blue-500" />Today's Orders</h3>
          <div className="space-y-2">
            <div className="flex justify-between"><span className="text-gray-600">Total</span><span className="font-bold">{orders.filter(o => sameDay(new Date(o.createdAt), new Date())).length}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Pending</span><span className="font-bold text-yellow-600">{orders.filter(o => o.status === "pending" && sameDay(new Date(o.createdAt), new Date())).length}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Completed</span><span className="font-bold text-green-600">{orders.filter(o => o.status === "completed" && sameDay(new Date(o.createdAt), new Date())).length}</span></div>
          </div>
        </div>

        {/* Inventory Status */}
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <h3 className="font-bold mb-3 flex items-center gap-2"><FaWarehouse className="text-purple-500" />Inventory Status</h3>
          <div className="space-y-2">
            <div className="flex justify-between"><span className="text-gray-600">Total Value</span><span className="font-bold">RWF {formatAmount(inventoryValue)}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Low Stock Items</span><span className={`font-bold ${lowStock.length > 0 ? 'text-red-600' : 'text-green-600'}`}>{lowStock.length}</span></div>
            {lowStock.length > 0 && (<div className="text-sm text-red-600 mt-2">{lowStock.slice(0, 2).map(item => (<div key={item.id}>• {item.name} ({item.qty} {item.unit})</div>))}</div>)}
          </div>
        </div>

        {/* Top Selling Items */}
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <h3 className="font-bold mb-3 flex items-center gap-2"><FaChartBar className="text-green-500" />Top Selling Items</h3>
          <div className="space-y-2">
            {topSellingItems.length === 0 ? <p className="text-gray-500 text-sm">No data available</p> : (
              topSellingItems.slice(0, 3).map(([name, qty], idx) => (
                <div key={name} className="flex items-center justify-between">
                  <span className="text-sm"><span className="font-bold text-gray-400">#{idx + 1}</span> {name}</span>
      ...
      {can.exportReports && (
        <div className="flex gap-3">
          <button onClick={exportPerformanceReport} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
            <FaFileCsv />Export Performance Report
          </button>
          <button onClick={exportDailyConsumption} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center gap-2">
            <FaDownload />Export Daily Consumption
          </button>
        </div>
      )}
    </div>
...
// ==================== ORDERS VIEW (Plan Mode step 30/15) ====================
function OrdersView({
  orders, orderSearch, setOrderSearch, statusFilter, setStatusFilter,
  calcOrderTotal, createOrder, startCooking, markServed, markCompleted,
  cancelOrder, receivePayment, deleteOrder, can, viewingOrder, setViewingOrder,
  menu, showToast
}) {
  const [draftItems, setDraftItems] = useState([]);
  const [customerName, setCustomerName] = useState("Walk-in");
  const [orderType, setOrderType] = useState("dine-in");
  const [planMode, setPlanMode] = useState(true); // default ON
  const [basis, setBasis] = useState("Month");    // Month | Half-month
  const STEP_MAP = { "Month": 30, "Half-month": 15 };
  const step = STEP_MAP[basis] || 30;

  const addToDraft = (m) => {
    if (!m.available) return showToast("Item not available", "warning");
    const exists = draftItems.find(i => i.menuItemId === m.id);
    if (exists) {
      setDraftItems(prev => prev.map(i => i.menuItemId === m.id ? { ...i, qty: i.qty + (planMode ? step : 1) } : i));
    } else {
      setDraftItems(prev => [...prev, { name: m.name, menuItemId: m.id, qty: planMode ? step : 1, unitPrice: m.price, notes: "" }]);
    }
  };
  const incQty = (id) => setDraftItems(prev => prev.map(i => i.menuItemId === id ? { ...i, qty: i.qty + (planMode ? step : 1) } : i));
  const decQty = (id) => setDraftItems(prev => prev.map(i => i.menuItemId === id ? { ...i, qty: Math.max(planMode ? step : 1, i.qty - (planMode ? step : 1)) } : i));
  const removeItem = (id) => setDraftItems(prev => prev.filter(i => i.menuItemId !== id));
  const draftTotal = draftItems.reduce((s, i) => s + i.unitPrice * i.qty, 0);

  const createFromDraft = () => {
    if (draftItems.length === 0) return showToast("Add items first", "warning");
    const newOrder = createOrder({ customerName, tableNumber: 1, orderType, items: draftItems });
    setDraftItems([]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <div className="grid md:grid-cols-3 gap-3">
          <div className="flex items-center gap-2 px-3 py-2 border rounded-lg">
            ...
            <input value={orderSearch} onChange={e => setOrderSearch(e.target.value)} placeholder="Search orders..." className="w-full outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border rounded-lg px-3 py-2">
            <option value="all">All Statuses</option>
            {["pending", "cooking", "served", "completed", "canceled"].map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <input id="planMode" type="checkbox" checked={planMode} onChange={(e) => setPlanMode(e.target.checked)} />
              <label htmlFor="planMode" className="text-sm font-medium">Plan Mode (step {step})</label>
            </div>
            {planMode && (
              <div className="flex items-center gap-2">
                <button onClick={() => setBasis("Month")} className={`px-3 py-1 rounded-lg border ${basis === "Month" ? "border-blue-600 bg-blue-50" : "border-gray-300"}`}>Month (30)</button>
                <button onClick={() => setBasis("Half-month")} className={`px-3 py-1 rounded-lg border ${basis === "Half-month" ? "border-blue-600 bg-blue-50" : "border-gray-300"}`}>Half (15)</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {/* Menu picker */}
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <h3 className="font-bold mb-3 flex items-center gap-2"><FaUtensils className="text-blue-600" />Menu</h3>
          <div className="grid md:grid-cols-2 gap-3">
            {menu.map(m => (
              <div key={m.id} className={`border rounded-lg p-3 ${!m.available ? 'opacity-50' : ''}`}>
                <div className="flex justify-between">
                  <div>
                    <div className="font-bold">{m.name}</div>
                    <div className="text-xs text-gray-500">RWF {formatAmount(m.price)}</div>
                  </div>
                  <button onClick={() => addToDraft(m)} disabled={!m.available} className="px-3 py-1 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">
                    Add
                  </button>
                </div>
              </div>
        ...
        {/* Draft */}
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <h3 className="font-bold mb-3 flex items-center gap-2"><FaClipboardCheck className="text-green-600" />Draft Order</h3>
          <div className="grid md:grid-cols-2 gap-3 mb-3">
            ...
            <div>
              ...
              <select value={orderType} onChange={e => setOrderType(e.target.value)} className="w-full border rounded-lg px-3 py-2">
          ...
          {draftItems.length === 0 ? (
            ...
            <div className="space-y-2">
              {draftItems.map(i => (
                <div key={i.menuItemId} className="flex items-center justify-between border rounded-lg p-2">
                  <div>
                    <div className="font-bold">{i.name}</div>
                    <div className="text-xs text-gray-500">RWF {formatAmount(i.unitPrice)} each</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => decQty(i.menuItemId)} className="w-8 h-8 rounded bg-gray-100 hover:bg-gray-200 flex items-center justify-center"><FaMinus /></button>
                    <div className="w-14 text-center font-bold">{i.qty}</div>
                    <button onClick={() => incQty(i.menuItemId)} className="w-8 h-8 rounded bg-blue-600 text-white hover:bg-blue-700 flex items-center justify-center"><FaPlus /></button>
                    <button onClick={() => removeItem(i.menuItemId)} className="w-8 h-8 rounded bg-red-600 text-white hover:bg-red-700 flex items-center justify-center"><FaTimes /></button>
                  </div>
                </div>
              ))}
              <div className="flex justify-between font-bold text-lg mt-2"><span>Total</span><span>RWF {formatAmount(draftTotal)}</span></div>
              <button onClick={createFromDraft} className="w-full py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Create Order</button>
              {planMode && <div className="text-xs text-green-600 mt-1">Plan Mode enabled: step is {step} per click</div>}
            </div>
          )}
        </div>
      </div>
      ...
      {/* Orders List */}
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <h3 className="font-bold mb-3 flex items-center gap-2"><FaClipboardList className="text-indigo-600" />Orders</h3>
        {orders.length === 0 ? (
          ...
          <div className="space-y-2">
            {orders.map(o => (
              <div key={o.id} className="border rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-bold">{o.orderNumber} • {o.customerName}</div>
                    <div className="text-xs text-gray-500">{new Date(o.createdAt).toLocaleString()} • {o.orderType}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold">RWF {formatAmount(calcOrderTotal(o))}</div>
                    <div className={`text-xs font-bold ${o.status === 'pending' ? 'text-yellow-600' : o.status === 'cooking' ? 'text-blue-600' : o.status === 'served' ? 'text-green-600' : o.status === 'completed' ? 'text-gray-700' : 'text-red-600'}`}>
                      {o.status.toUpperCase()}
                    </div>
                  </div>
                </div>

                <div className="text-xs text-gray-500 mt-1">Items: {o.items.map(i => `${i.name} x${i.qty}`).join(", ")}</div>

                <div className="flex flex-wrap gap-2 mt-2">
                  {can.confirm && o.status === "pending" && <button onClick={() => startCooking(o.id)} className="px-3 py-1 bg-yellow-500 text-white rounded-lg text-sm hover:bg-yellow-600">Confirm</button>}
                  {can.serve && o.status === "cooking" && <button onClick={() => markServed(o.id)} className="px-3 py-1 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Serve</button>}
                  {can.complete && (o.status === "served" || o.status === "cooking") && <button onClick={() => markCompleted(o.id)} className="px-3 py-1 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700">Complete</button>}
                  {can.receivePayment && !o.paid && <button onClick={() => receivePayment(o.id, "cash")} className="px-3 py-1 bg-purple-600 text-white rounded-lg text-sm hover:bg-purple-700">Receive Payment</button>}
                  <button onClick={() => cancelOrder(o.id)} className="px-3 py-1 bg-gray-200 rounded-lg text-sm hover:bg-gray-300">Cancel</button>
                  {can.deleteOrder && <button onClick={() => deleteOrder(o.id)} className="px-3 py-1 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700">Delete</button>}
                </div>
              </div>
            ))}
          </div>
...
// ==================== MENU VIEW ====================
function MenuView({ menu, toggleMenuAvailability, updateMenuPrice, addMenuItem, can, showToast }) {
  const [newItem, setNewItem] = useState({ name: "", price: 0, category: "Main", preparationTime: 15, calories: 0 });

  const add = () => {
    if (!can.editMenu) return showToast("Permission denied", "error");
    if (!newItem.name.trim()) return showToast("Enter item name", "warning");
    if (newItem.price <= 0) return showToast("Enter valid price", "warning");
    addMenuItem(newItem);
    setNewItem({ name: "", price: 0, category: "Main", preparationTime: 15, calories: 0 });
  };

  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <h3 className="font-bold mb-3">Add Menu Item</h3>
        <div className="grid md:grid-cols-5 gap-3">
          <input className="border rounded-lg px-3 py-2" placeholder="Name" value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} />
          <input className="border rounded-lg px-3 py-2" type="number" placeholder="Price" value={newItem.price || ""} onChange={e => setNewItem({ ...newItem, price: Number(e.target.value || 0) })} />
          <select className="border rounded-lg px-3 py-2" value={newItem.category} onChange={e => setNewItem({ ...newItem, category: e.target.value })}>
            <option>Main</option><option>Starter</option><option>Beverage</option>
          </select>
          <input className="border rounded-lg px-3 py-2" type="number" placeholder="Prep (min)" value={newItem.preparationTime || ""} onChange={e => setNewItem({ ...newItem, preparationTime: Number(e.target.value || 0) })} />
          <button onClick={add} className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Add</button>
        </div>
      </div>
          ...
          {menu.map(m => (
            <div key={m.id} className="border rounded-lg p-3">
              <div className="flex justify-between items-start">
                <div>
                  <div className="font-bold">{m.name}</div>
                  <div className="text-xs text-gray-500">{m.category} • Prep {m.preparationTime} min</div>
                </div>
                <span className={`text-xs font-bold px-2 py-1 rounded ${m.available ? 'bg-green-100 