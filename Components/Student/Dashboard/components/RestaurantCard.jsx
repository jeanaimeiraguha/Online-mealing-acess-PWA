...
import { FaHeart, FaRegHeart, FaStar, FaWalking, FaMapMarkerAlt, FaInfoCircle, FaShoppingCart, FaLock } from 'react-icons/fa';
...
const RestaurantCard = ({ restaurant, onToggleFav, onShowInfo, onSubscribe, onOrder, isSubscribed, isLocked, onUnlock }) => {
  const [imageLoaded, setImageLoaded] = useState(false);
  const planEntries = Object.entries(restaurant.priceInfo || {});
  const rating = restaurant.rating || 4.5;
  const lowest = planEntries.length > 0 ? planEntries.reduce((lowest, current) => (current[1] < lowest[1] ? current : lowest), planEntries[0]) : null;
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [isHovering, setIsHovering] = useState(false);
  const intervalRef = useRef(null);

  useEffect(() => { // Image slideshow on hover
    if (isHovering) {
      intervalRef.current = setInterval(() => {
        setCurrentImageIndex(prevIndex => (prevIndex + 1) % (restaurant.images?.length || 1));
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-[#1a1a15] rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-2xl border border-gray-200 dark:border-white/10 overflow-hidden group relative select-none"
    ...
    >
      ...
      <div className={`relative h-40 sm:h-48 overflow-hidden ${isLocked ? 'blur-sm' : ''}`}>
        <div className={`absolute inset-0 bg-gradient-to-b from-transparent to-black/40 z-10 ${!imageLoaded ? 'animate-pulse bg-gray-300 dark:bg-gray-700' : ''}`} />
        <img
          src={restaurant.images ? restaurant.images[currentImageIndex] : restaurant.image}
          alt={restaurant.name}
          ...
          onLoad={() => setImageLoaded(true)} loading="lazy" draggable="false"
        />
      ...
      {isLocked && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm z-30">
          <FaLock className="text-white text-4xl mb-4" />
          <h3 className="text-white font-bold text-lg">Card is Locked</h3>
          <p className="text-white/80 text-sm">Unlock to see details and order</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
          {planEntries?.map(([period, amount]) => (
            <div key={period} className={`p-2 rounded-lg border ${lowest?.period === period ? 'bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700' : 'bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700'}`}>
              ...
              {lowest?.period === period && <div className="text-[10px] sm:text-xs text-green-600 dark:text-green-400 font-semibold mt-1">Best value</div>}
            </div>
          ))}
        </div>

        <div className="flex gap-2">
          <motion.button whileTap={tapAnimation} onClick={() => onShowInfo(restaurant)} className="flex-1 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold py-2.5 sm:py-3 rounded-xl transition-all flex items-center justify-center gap-2 text-sm">
          ...
          <motion.button
            ...
            onClick={() => {
              if (!isSubscribed) {
                onSubscribe(restaurant); // Redirect to subscription page
              } else if (isLocked) {
                onUnlock(); // Prompt to unlock
              } else {
                onOrder(restaurant); // Allow ordering
              }
            }}
            className="flex-[2] bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-2.5 sm:py-3 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={isSubscribed && isLocked}
          >
            <FaShoppingCart />
            <span>
              {!isSubscribed ? "Subscribe Now" : isLocked ? "Unlock to Order" : "Order Now"}
            </span>
          </motion.button>
        </div>
      </div>
