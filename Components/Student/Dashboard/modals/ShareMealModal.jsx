  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <motion.div variants={modalMotion} initial="initial" animate="animate" exit="exit" onClick={e => e.stopPropagation()} className="w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl p-4 sm:p-6 shadow-2xl m-4">
        <h3 className="text-xl sm:text-2xl font-bold text-center mb-6 text-gray-900 dark:text-white">Share Meals</h3>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={studentId} onChange={(e) => setStudentId(e.target.value)} placeholder="Enter student ID" className="w-full px-4 py-2.5 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
          </div>

          <div>
            <label className="text-sm text-gray-600 dark:text-gray-400 mb-2 block">Number of Meals</label>
            <div className="flex items-center justify-center gap-4 bg-gray-100 dark:bg-gray-800 rounded-xl p-2 sm:p-4">
              <motion.button whileTap={tapAnimation} onClick={() => setMealsToShare(m => Math.max(1, m - 1))} className="w-10 h-10 sm:w-12 sm:h-12 bg-white dark:bg-gray-700 rounded-xl flex items-center justify-center font-bold">
                <FaMinus />
              </motion.button>
              <span className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white w-12 text-center">{mealsToShare}</span>
              <motion.button whileTap={tapAnimation} onClick={() => setMealsToShare(m => Math.min(remainingMeals, m + 1))} className="w-10 h-10 sm:w-12 sm:h-12 bg-purple-600 text-white rounded-xl flex items-center justify-center font-bold">
                <FaPlus />
              </motion.button>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Add a message..." rows={2} className="w-full px-4 py-2.5 sm:py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
          </div>
