...
const BiometricSetup = ({ onSetupComplete, onSkip, username, role }) => {
  const [isSettingUp, setIsSettingUp] = useState(false);
  const [setupMethod, setSetupMethod] = useState(""); // "fingerprint" or "passcode"

  const handleFingerprintSetup = async () => {
    setIsSettingUp(true);
    setSetupMethod("fingerprint");

    const result = await WebAuthnHelper.registerBiometric(
      username,
      `${role === "student" ? "Student" : "Restaurant"}: ${username}`
    );
    ...
    if (result.success) {
      ...
      toast.error(`Failed to setup fingerprint: ${result.error}`);
      setSetupMethod("");
    }
  };

  const handlePasscodeSetup = () => {
    setSetupMethod("passcode");
    // In a real implementation, this would set up a passcode
    // For demo, we'll simulate success
    setTimeout(() => {
      toast.success("Passcode authentication enabled successfully!");
      onSetupComplete(true);
    }, 1000);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-100 relative py-6 font-sans">
      <Toaster position="top-center" />

      <div className="text-center mt-12">
        <div className="bg-gradient-to-br from-purple-100 to-purple-200 p-4 rounded-full inline-block mb-4">
          <FaFingerprint className="text-purple-600 text-4xl" />
        </div>
        <h2 className="text-3xl font-bold text-gray-800 mb-2">Secure Your Account</h2>
        <p className="text-gray-600 text-lg max-w-md">
          Choose your preferred security method for quick and secure logins
        </p>
      </div>

      <div className="w-full max-w-md mt-8 bg-white p-8 rounded-2xl shadow-2xl">
        {!setupMethod ? (
          <>
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full p-4 border-2 border-gray-200 rounded-xl hover:border-purple-500 hover:bg-purple-50 transition-all duration-300 flex items-center justify-between group"
              >
                <div className="flex items-center space-x-4">
                  <div className="bg-purple-100 p-3 rounded-full">
                    <FaFingerprint className="text-purple-600 text-2xl" />
                  </div>
                  <div className="text-left">
                    <h3 className="font-semibold text-gray-800">Use Fingerprint</h3>
                    <p className="text-gray-500 text-sm">Fast and secure biometric login</p>
                  </div>
                </div>
                <FaArrowRight className="text-gray-400 group-hover:text-purple-600 transition-all" />
              </button>
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="text-left">
                    <h3 className="font-semibold text-gray-800">Use Passcode</h3>
                    <p className="text-gray-500 text-sm">Set a secure numeric passcode</p>
                  </div>
                </div>
                <FaArrowRight className="text-gray-400 group-hover:text-blue-600 transition-all" />
              </button>
            </div>
          ...
          <div className="text-center py-8">
            {setupMethod === "fingerprint" ? (
              <>
                {isSettingUp ? (
                  <>
                    <BiScan className="text-purple-600 text-5xl mb-4 animate-pulse mx-auto" />
                    <h3 className="text-xl font-semibold text-gray-800 mb-2">Setting up Fingerprint</h3>
                    <p className="text-gray-600">Follow your device's prompt to register your fingerprint...</p>
                    <div className="mt-4">
                      <FaSpinner className="animate-spin text-purple-600 text-2xl mx-auto" />
                    </div>
                  </>
                ) : null}
              </>
              ...
              <>
                <FaLock className="text-blue-600 text-5xl mb-4 mx-auto" />
                <h3 className="text-xl font-semibold text-gray-800 mb-2">Passcode Setup Complete</h3>
                <p className="text-gray-600">Your passcode has been set up successfully!</p>
              </>
            )}
          </div>
        )}
      ...
      <div className="mt-6 text-center text-gray-500 text-sm">
        <p>You can change this setting anytime in your account preferences</p>
      </div>
    </div>
  );
};
...
const SignUpPage = () => {
  ...
  // New states for email verification and biometric setup
  const [emailVerificationStep, setEmailVerificationStep] = useState(false);
  const [biometricSetupStep, setBiometricSetupStep] = useState(false);
  ...
  const handleChange = (e) => {
    ...
    if (name === "confirmPassword") setFieldStatus((prev) => ({ ...prev, confirmPassword: value === formData.password }));
  };
  ...
  const handleBiometricSetupComplete = (success) => {
    if (success) {
  ...
  const handleBiometricSetupSkip = () => {
    // Skip biometric setup and complete registration
    handleCompleteRegistration();
  };
    ...
    try {
      if (isLogin) {
        ...
        if (!isSuccess) {
          ...
          const passwordOk = formData.password === "password123"; // Demo password
          if (passwordOk) {
            isSuccess = true;
          } else {
          ...
          <>
            ...
            <div className="relative group">
              ...
              <input
                ...
                onChange={handleChange}
                className={`w-full px-10 pr-12 py-3 rounded-full border-2 transition-all focus:ring-2 focus:ring-blue-500 ${
              ...
              <div className="group relative inline-block">
                <FaQuestionCircle className="absolute right-10 top-1/2 -translate-y-1/2 text-yellow-500 cursor-help" />
            ...
            {!isLogin && (
              <div className="relative group">
                ...
                {fieldStatus.confirmPassword && (
                  <FaCheck className="absolute right-10 top-1/2 -translate-y-1/2 text-green-500" />
                )}
              </div>
            )}
